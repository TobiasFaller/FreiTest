...
In case you have problems (as I did) with the breakID CMS preproseccor - at the end is an explanation on how to install it.
...
BreakID
-----
With the following steps you can get BreakID and install it (tested on Ubuntu).
...
Then you should be able to run `cmake (-DSTATICCOMPILE=ON) ..` at your CMS repository. For me it was necessary to delete all files from the build folder.
