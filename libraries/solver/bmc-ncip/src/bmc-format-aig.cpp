...
std::tuple<AigProblem, BmcProblem> AigProblemBuilder::Build() {
	...
	std::vector<AigEdge> inputs { this->inputs };
	std::vector<AigEdge> latches { this->latches };
	std::vector<AigEdge> ands { this->ands };
	std::vector<AigEdge> outputs { this->outputs };
	std::vector<AigEdge> bads { (this->bads.size() == 0) ? this->outputs : this->bads };
	std::vector<AigEdge> constraints { this->constraints };
	...
	AigProblem aigProblem { nodes, inputs, latches, ands, outputs, bads, constraints };
