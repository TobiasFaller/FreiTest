...
#ifndef BOOST_PROPERTY_TREE_JSONC_PARSER_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_JSONC_PARSER_HPP_INCLUDED
...
#include "Io/JsoncParser/ReadDetail.hpp"
#include "Io/JsoncParser/WriteDetail.hpp"

namespace boost { namespace property_tree { namespace jsonc_parser
{
    ...
    void read_jsonc(std::basic_istream<
                       typename Ptree::key_type::value_type
                   > &stream,
                   Ptree &pt)
    {
        detail::read_jsonc_internal(stream, pt, std::string());
    }
     ...
     */
    template<class Ptree>
    void read_jsonc(const std::string &filename,
                   Ptree &pt,
                   const std::locale &loc = std::locale())
    {
        ...
        stream.imbue(loc);
        detail::read_jsonc_internal(stream, pt, filename);
    }
     ...
     */
    template<class Ptree>
    void write_jsonc(std::basic_ostream<
                        typename Ptree::key_type::value_type
                    > &stream,
                    const Ptree &pt,
                    bool pretty = true)
    {
        write_jsonc_internal(stream, pt, std::string(), pretty);
    }
     ...
     */
    template<class Ptree>
    void write_jsonc(const std::string &filename,
                    const Ptree &pt,
                    const std::locale &loc = std::locale(),
                    bool pretty = true)
    {
        ...
        if (!stream)
            BOOST_PROPERTY_TREE_THROW(json_parser_error(
                "cannot open file", filename, 0));
        stream.imbue(loc);
        write_jsonc_internal(stream, pt, filename, pretty);
    }
...
{
    using jsonc_parser::read_jsonc;
    using jsonc_parser::write_jsonc;
    using jsonc_parser::json_parser_error;
} }
