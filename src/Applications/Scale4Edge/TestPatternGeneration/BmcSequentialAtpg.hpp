 ...
 * from the specified input / output ports.
 ...
 *       - Tag all circuit parts to be encoded with
 *         GoodTag (good, no fault) and BadTag (bad, fault)
 *       - Encode the tagged circuit parts by using the logic generator
 *     - Solve the CNF formula by using a BMC-solver
 *     - When BMC-result is "reachable" (satisfiable):
 ...
 *             - Add the fault to testable fault list
 *     - When BMC-result is "unreachable" (unsatisfiable):
...
class BmcSequentialAtpg:
	public SequentialAtpgBase<FaultModel, FaultList>
{
public:
	BmcSequentialAtpg(void);
	virtual ~BmcSequentialAtpg(void);
